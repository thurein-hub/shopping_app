<?php
	...
	$stmt = $pdo->prepare("INSERT INTO sale_orders(user_id,total_price,order_date) VALUES(:user_id,:total_price,:orderdate)");
	$result = $stmt->execute(
		array(
			':user_id'=>$userId,
			':total_price'=>$total,
	...
	if($result){
		...
		foreach($_SESSION['cart'] as $key=>$qty){
			...
			$stmt = $pdo->prepare("INSERT INTO sale_order_detail(sale_order_id,product_id,quantity)
									VALUES(:saleorderId,:pro_id,:qty)");
			$result=$stmt->execute(
					array(
						':saleorderId'=>$sale_orderId,
						':pro_id'=>$id,
						':qty'=>$qty
